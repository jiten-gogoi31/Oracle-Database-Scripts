...
1. create smallfile tablespace users2 datafile size 1G autoextend on maxsize 10G;
2. create temporary tablespace temp01;

Please note, Don’t use smallfile tablespaces because you can’t resize smallfile tablespaces with Amazon RDS for Oracle. However, you can add a datafile to a smallfile tablespace.

1. alter tablespace users2 add datafile size 100000M autoextend on next 250m maxsize UNLIMITED;
2. Setting the Default Tablespace
...
1. exec rdsadmin.rdsadmin_util.add_logfile(p_size => ‘Size in M‘);
2. exec rdsadmin.rdsadmin_util.drop_logfile(grp => Number of groups);
